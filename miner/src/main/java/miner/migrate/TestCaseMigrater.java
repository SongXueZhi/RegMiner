...
//		}
     ...
     * @param bfcDir
     */
    public void copyTestCase(@NotNull PotentialRFC pRFC, File tDir, File bfcDir) {
        ...
        for (TestFile tc : tcs) {
            ...
            if (tpath == null) { // 如果类没有,就直接迁移过去
                ...
                for (Map.Entry<String, RelatedTestCase> entry : tc.getTestMethodMap().entrySet()) {
                    ...
                    JdtMethodRetriever mr = new JdtMethodRetriever();
                    unitTo.accept(mr);
                    ...
                    for (ImportDeclaration impo : imports) {
                        ...
                        String pattern = str.substring(str.lastIndexOf(".") + 1);
    ...
    public boolean whetherImportExits(@NotNull List<ImportDeclaration> imports, ImportDeclaration im) {
    ...
    public boolean whetherMehothdExits(@NotNull List<MethodDeclaration> mLsit, MethodDeclaration md) {
    ...
    public void copyAndPruneNoUsedMethod(String javaFilepath, String classFile, File tDir, @NotNull File bfcDir,
                                         @NotNull Map<String, RelatedTestCase> methodMap) {
    ...
    public String pruneField(String content) throws MalformedTreeException, BadLocationException {
        ...
        JdtFieldRetriever fr = new JdtFieldRetriever();
        unit.accept(fr);
    ...
    public void copyToTarget(@NotNull PotentialRFC pRFC, File targerProjectDirectory) {
        // copy
        String targetPath = null;
        for (TestFile testFile : pRFC.getTestCaseFiles()) {
            File file;
            file = new File(Conf.TMP_FILE + File.separator + pRFC.getCommit().getName() + File.separator
                    + testFile.getNewPath());
            // 测试文件是被删除则什么也不作。
