...
public class BICFinder {
     ...
     * @return
     */
    public Regression searchBIC(PotentialBFC pRFC) {
        ...
        if (a >= 0 || (falPoint - passPoint) == 1) {
            ...
            return new Regression(UUID.randomUUID().toString(),
                    Configurations.PROJRCT_NAME + "_" + bfcId, bfcId,
                    bfcpId, bic,
                    working,
                    testcaseString, 0);
        } else if (passPoint >= 0 && falPoint - passPoint > 1) {
            ...
            return new Regression(UUID.randomUUID().toString(),
                    Configurations.PROJRCT_NAME + "_" + bfcId, bfcId, bfcpId, arr[falPoint], arr[passPoint],
                    testcaseString, 1);
        }
        exec.setDirectory(new File(Configurations.PROJECT_PATH));
        new SycFileCleanup().cleanDirectory(bfcFile);
    ...
    public List<String> revListCommand(String commitId) {
        synchronized (Configurations.META_PATH) {
            exec.setDirectory(new File(Configurations.META_PATH));
