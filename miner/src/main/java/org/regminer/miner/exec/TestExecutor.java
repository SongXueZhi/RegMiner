...
public class TestExecutor extends Executor {
    ...
    // 请注意以最小的单元运行任务
    public MigrateItem.MigrateFailureType execTestWithResult(String cmd) {
        Process process = null;
        Timer t = null;
        InputStreamReader inputStr = null;
        BufferedReader bufferReader = null;
        ...
        try {
            ...
            while ((line = bufferReader.readLine()) != null) {
                ...
                if (line.contains("build success")) {//there may exist the case that: build success and test failure
                    ...
                    return MigrateItem.MigrateFailureType.NoTests;
                } else if (line.contains("build failure")) {
                    if (testCE) {
                        return MigrateItem.MigrateFailureType.CompilationFailed;
                    } else {
                        maySuccess = false;
                        mayFail = true;
                    }
                } else if (line.contains("test failure")) {
                    return MigrateItem.MigrateFailureType.NONE;
                }
            }
            if (testCE) {
                return MigrateItem.MigrateFailureType.CompilationFailed;
        ...
        if (maySuccess && !mayFail) {
            return MigrateItem.MigrateFailureType.TESTSUCCESS;
        }
        return MigrateItem.MigrateFailureType.NONE;
    }
