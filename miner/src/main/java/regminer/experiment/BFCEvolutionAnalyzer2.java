...
public class BFCEvolutionAnalyzer2 extends Migrator {
    ...
    public void handleTask(List<String> filter) throws Exception {
        ...
        for (PotentialRFC pRFC : potentialRFCList) {
            ...
            double score = trackCodeBlocks(codeBlockMap,bfcDir);
            emptyCache(commitID);
    ...
    public Double trackCodeBlocks(Map<String, CodeBlock> codeBlockMap, File bfcDir) {
        Double sum = 1D;
        for (Map.Entry<String, CodeBlock> entry : codeBlockMap.entrySet()) {
            CodeBlock block = entry.getValue();
            int n = gitTracker.trackCodeBlockByLogl(block.getStartLine(), block.getStopLine(), block.getNewPath(), bfcDir,block.isIfMethod());
            Double prob = 0.1*Math.pow((1+block.getStopLine()-block.getStartLine()),-0.5);
            sum*=Math.pow((1-prob),n);
        }
        return 1-sum;
    }
    ...
    public static void main(String[] args) throws Exception {
        BFCEvolutionAnalyzer2 analyzer2 = new BFCEvolutionAnalyzer2();
        analyzer2.prepare();
